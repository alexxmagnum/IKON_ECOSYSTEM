/**
 * @motanos/booking-lifecycle — Booking lifecycle / operations motor (provisional).
 *
 * MotanOS Core → Shared Engines → @motanos/booking-lifecycle
 *
 * Holds the historical booking motor (services, queries, repositories,
 * and related operational boundaries). Domain existence contracts live in
 * @motanos/booking (DEC-BOOKING-BOUNDARY-001).
 *
 * Must not depend on customer implementations, concrete domains,
 * persistence packages, or payment vendors.
 */

export const BOOKING_LIFECYCLE_ENGINE = "@motanos/booking-lifecycle" as const;

/** @deprecated Prefer BOOKING_LIFECYCLE_ENGINE — kept for transitional imports. */
export const BOOKING_ENGINE = BOOKING_LIFECYCLE_ENGINE;
